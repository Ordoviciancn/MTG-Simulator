...
MTG Simulator 是基于 Forge 规则引擎的万智牌双人桌面模拟器。客户端采用 Electron、React 和 TypeScript，支持本机主持、远程房间、牌组管理以及 BO1 / BO3 对局。

[下载安装包](https://github.com/Ordoviciancn/MTG-Simulator/releases/latest) · [使用指南](docs/一键联机与完整安装包.md) · [开发文档](docs/开发与构建.md)

## 更新通告 · v0.1.6（2026-09-15）

- 首局直接随机分配先后手，移除投掷动画；BO3 后续由上一局败者选择先后手。
- 展示战场指示物与装备、灵气的贴附关系，支持合法可见的牌库顶和特殊区域出牌。
- 优化大量候选项的搜索、汉化加载进度和衍生物卡图回退。
- 提供内置 Forge、JDK 17 与 Cloudflared 的 Windows x64 完整安装包。

- 新增可选**一键汉化**：牌组工坊点击后，大厅、牌表与对局内切换中文牌名与卡图（默认英文显示，可随时关闭）。
- 新增右侧半圆箭头**堆叠/历史面板**：有牌或异能进堆叠时自动展开，处理结束自动收回，也可随时手动查看真实堆叠与最近结算记录。
- 修复汉化查询中途停止后无法继续的问题，失败请求会自动重试。
...
1. 从 [Releases](https://github.com/Ordoviciancn/MTG-Simulator/releases/latest) 下载并运行安装程序。
2. 在牌组工坊创建或导入牌组并保存。
...
完整文档见 [文档索引](docs/README.md)。提交 [Issue](https://github.com/Ordoviciancn/MTG-Simulator/issues) 时，请提供软件版本、复现步骤、相关牌名及错误信息；不要附带房间凭据或私人数据。
