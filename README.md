...
> 项目定位：轻量级数字牌桌，不是完整自动规则引擎。默认手动模式由玩家裁定；可切换到有限的辅助结算模式，见下方范围。
...
界面使用原创绘制的石质竞技场背景、双方生命面板、底部手牌、中央阶段条和可收起的牌桌工具。未加载卡图时可直接进入房间；卡图获取为可选操作。

在右侧「规则辅助」选择「辅助结算」，点击手牌后选择目标，再点击「支付费用并施放」。服务端自动选择并横置基本地支付费用；「结算堆叠顶」按后进先出处理。每回合最多出一张地，结束回合会重置下一位玩家的永久物并抓一张牌。切换模式须由当前回合玩家操作。

当前支持五种基本地、Grizzly Bears、Hill Giant、Divination、Sacred Nectar、Lightning Bolt（仅以玩家为目标）和 Counterspell。辅助模式拒绝未知永久物、衍生物、指示物与依附等尚未支持的组合；复杂对局使用手动模式。允许手动抓牌和洗牌，因此这是合作式规则辅助，并非防作弊或完整赛事裁判。

尚未实现战斗、优先权交接、状态动作、输赢判定与完整 Oracle 语义，也尚未接入独立 Python Oracle 编译器。服务重启会清空内存房间。

验证命令：`pnpm test`（规则行为与双客户端集成）和 `pnpm build`。生产端口默认 8787，可通过环境变量 `PORT` 修改。
