...
> 项目正在全面重构为 Forge 权威规则驱动的 MTGA 对局客户端，详见 [重构审计与差距矩阵](docs/全面重构审计与差距矩阵.md)。以下功能列表描述当前旧手动界面，不代表重构目标已经完成。`pnpm forge:human-smoke` 已能验证真实双 Human Match 到达选择窗口，但不验证完整对局。
