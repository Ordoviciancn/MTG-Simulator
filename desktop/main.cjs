const { app, BrowserWindow, Menu, dialog, utilityProcess, ipcMain } = require('electron');
...
async function start() {
  ...
  server = utilityProcess.fork(path.join(root, 'server-entry.mjs'), [], {
    cwd: root, env: { ...process.env, PORT: process.env.MTG_SERVER_PORT||'0', HOST: config.mode==='host'?'0.0.0.0':'127.0.0.1', JAVA_HOME: config.javaHome, FORGE_WORKSPACE_ROOT: config.workspace,FORGE_REMOTE_CLIENT_ONLY:config.mode==='remote'?'1':'0' },
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...
    } }, {label:'服务器连接地址',click:()=>dialog.showMessageBox({message:config.mode==='host'?'本机主持已启用':'远程客户端模式',detail:config.mode==='host'?Object.values(os.networkInterfaces()).flat().filter(ip=>ip&&ip.family==='IPv4'&&!ip.internal).map(ip=>`http://${ip.address}:${port}`).join('\n')+'\n同一网络的玩家可填写以上地址。公网访问需服务器网络配置。':'请在大厅填写远程服务器地址。'})}, { label: '运行环境位置', click: () => dialog.showMessageBox({ message: 'Forge 运行环境', detail: `项目：${config.workspace||'未启用本机引擎'}\nJDK：${config.javaHome||'无需本机安装'}\n设置文件：${path.join(app.getPath('userData'), 'runtime.json')}` }) }, { type: 'separator' }, { role: 'quit', label: '退出' }] },
