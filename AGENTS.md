...
- `desktop/` 管理 Electron 宿主和打包。`pnpm desktop:build` 输出 `release/MTG Simulator-win32-x64/MTG Simulator.exe`，须保留整个目录；构建输出不提交。远程玩家无需 Forge/JDK，主持者需要外部固定版本 Forge 工作区、JDK 和 Git，配置见 `desktop/桌面版说明.md`。
