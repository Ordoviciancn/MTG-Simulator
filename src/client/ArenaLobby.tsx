import {LocalizedName,setChineseCards,useChineseCards} from './LocalizedCard';
...
export function ArenaLobby(props:ArenaLobbyProps){
  ...
  const localized=useChineseCards();
  ...
  return <section className="arena-lobby" aria-label="对局大厅">
    <header className="lobby-hero"><div><span className="lobby-eyebrow">MTG SIMULATOR · FORGE</span><h1>每一场对局，从这里开始</h1><p>整理你的牌组，选择服务器，迎接下一位对手。</p></div><span className={`lobby-status ${props.connected?'online':''}`} role="status">{props.connected?'已连接服务器':'正在等待连接'}</span></header>
