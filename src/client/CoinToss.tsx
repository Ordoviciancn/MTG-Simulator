...
export function CoinToss({winner,onComplete}:{winner:string;onComplete:()=>void}){
  const [landed,setLanded]=useState(false);
  useEffect(()=>{
    const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
    const landing=setTimeout(()=>setLanded(true),reduced?0:1700);
    const end=setTimeout(onComplete,reduced?800:2900);
    return()=>{clearTimeout(landing);clearTimeout(end);};
  },[onComplete]);
